...
dat <- dat %>% mutate(learn= recode(respect, "yes" = "1", "no" = "0"))
...
model1a <- lm(ladder ~ BasicNeeds, data = dat)

model1b <- lm(ladder ~ BasicNeeds + PsychNeeds, data = dat)

 model1c <- lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences, data = dat)

 model1d <- lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard, data = dat)

 model1e <-lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard + logIncome, data = dat)

# dat <- dat %>% rename(study_site = dataSet)
# 
# #Fit a hierarchical regression model, adding study_site as a predictor
#ladder_with_site <- lm(ladder ~ BasicNeeds + PsychNeeds + LuxuryConveniences + livingStandard + logIncome + study_site, data = dat)
# 
# model1b <- lm(ladder ~ BasicNeeds)
# 
# model2a <- lm(PositiveFeelings ~  + predictor2, data = dat)
# 
# model2b <- lm(PositiveFeelings ~ predictor1 + ___________+ study_site, data = dat)
# 
# model3a <- lm(NegativeFeelings ~ 
# model3b <- lm(NegativeFeelings ~ predictor1 + _________ + study_site, data = dat)

#predictor_1 = BasicNeeds
#predictor 2 = PsychNeeds
#predictor3 = LuxuryConveniences 
#predictor 4 = LivingStandard (is this a separate predictor? need to ask)
#predictor 5 = logIncome
```
